...
def perception_step(Rover):
    ...
    obs_wx, obs_wy = pix_to_world(obstaclesx, obstaclesy,Rover.pos[0],  Rover.pos[1], Rover.yaw, wm.shape[0], scale-3)
    ...
    if (Rover.pitch < 0.5 or Rover.pitch > 359.5) and (Rover.roll < 0.5 or Rover.roll > 359.5):
        ...
        Rover.worldmap[rover_wy, rover_wx, 2] =255
        # if the pixels are blue then remove its true value in the red channel 
        blue_pixs=Rover.worldmap[:, :, 2]>0
        Rover.worldmap[blue_pixs, 0]=0
    ...
    if len(rocksx) > 5:
        ...
        rock_distance, rock_angle = to_polar_coords(x_pixel=rocksx,
                                                    y_pixel=rocksy)
